	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			PreparedStatement psmt = conn.prepareStatement(
					"INSERT INTO add_employee(employeeId,firstName,lastName,email,department,gender,dob,contact,country,address,city,password , Joining_Date ,Joining_day) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");

			String joining_Date = dateFormat.format(currentDate);
			String joining_Day = dayFormat.format(currentDate);
			...
			psmt.setString(13, joining_Date);
			psmt.setString(14, joining_Day);
